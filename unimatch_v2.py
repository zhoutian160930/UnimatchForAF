...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                pred_u_w = model_ema(img_u_w).detach()#ema
                conf_u_w = pred_u_w.softmax(dim=1).max(dim=1)[0]#conf
            ...
            pred_x = model(img_x)
            pred_u_s1, pred_u_s2 = model(torch.cat((img_u_s1, img_u_s2)), comp_drop=True).chunk(2)#互补
